package edu.berkeley.cs160.gsale;

public class Server {
	public static final String URL = "http://alvinyuan.pythonanywhere.com";
	public static final String POST_SALE_SUFFIX = "/sale";
	public static final String GET_ALL_SALES_SUFFIX = "/sales";
	public static final String LOG_IN_SUFFIX = "/user";
	public static final String POST_PHOTO_SUFFIX = "/photo";
	public static final String GET_PHOTO_SUFFIX = "/photo";
	public static final String GET_ALL_PHOTOS_SUFFIX = "/photos";
	public static final String POST_SALE_PHOTO_SUFFIX = "/salephoto";
	public static final String GET_SALE_PHOTOS_SUFFIX = "/salephotos";
	public static final String POST_MESSAGE_SUFFIX = "/message";
	public static final String GET_ALL_MESSAGES_SUFFIX = "/messages";
}
